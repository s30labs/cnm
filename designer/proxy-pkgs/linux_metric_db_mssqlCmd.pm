package linux_metric_db_mssqlCmd;
# /opt/cnm/designer/gconf-proxy -m linux_metric_db_mssqlCmd -p db-mssqlCmd
#---------------------------------------------------------------------------
...
# 0 METRICA  1=>APP
#---------------------------------------------------------------------------
$linux_metric_db_mssqlCmd::CFG = 0;
$linux_metric_db_mssqlCmd::SCRIPT_NAME = 'linux_metric_db_mssqlCmd.pl';
...
#---------------------------------------------------------------------------
%linux_metric_db_mssqlCmd::SCRIPT = (

	'__SCRIPT__' => $linux_metric_db_mssqlCmd::SCRIPT_NAME,
   '__EXEC_MODE__' => 1,
   '__SCRIPT_DESCRIPTION__' => '',  # creo que sobra ¿?
   '__PROXY_TYPE__' => 'linux',
   '__CFG__' => $linux_metric_db_mssqlCmd::CFG,
   '__PROXY_USER__' => 'www-data',
   '__PROXY_PWD__' => 'cnm123',
   '__TIMEOUT__' => 30,

#./linux_metric_db_mssqlCmd.pl -host 192.168.62.127 -db IMODB -user ldapuserp -pwd ld4p1serp -sqlcmd "SET NOCOUNT ON;SELECT COUNT([UserAccount]) AS '001' FROM [IMODB].[dbo].[vwLDAPAcces] FOR JSON AUTO" -tag '001' -label 'Numero de usuarios'
	...
	# ----------------------------
	'__TIP__'  => {
		'__DESCR_TIP__' => 'Allows SQL command execution on Microsoft SQL Server. Useful for creating custom metrics returning a numeric value.
...
',
		'__ID_REF__' => $linux_metric_db_mssqlCmd::SCRIPT_NAME
	}
);
...
@linux_metric_db_mssqlCmd::METRICS = (

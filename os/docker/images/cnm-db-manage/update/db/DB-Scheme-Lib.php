...
global $enlace;
   ...
   _debug("Funcion:connectDB||{$db_params['database']}||Tiempo:$tiempo",__LINE__,'INF','delete_from_table');
	...
	foreach($tableData as $tableEntryToDelete){
      ...
      if ($enlace->errno != 0) {
         $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
         // _debug("No se ha podido borrar la entrada en la tabla $tableName.||QUERY=>$sqlDel || $err_str",__LINE__,'ERR','delete_from_table');
      }
      else{
...
global $enlace;
   ...
   // NOS CONECTAMOS A LA BBDD
	...
	$timeout = 2;
	$enlace = mysqli_init();
	$enlace->options( MYSQLI_OPT_CONNECT_TIMEOUT, $timeout ) ||
		_debug('mysqli_options error '.$link->error,__LINE__,'DBG','connectDB');

	$enlace->real_connect($db_params['hostspec'], $db_params['username'], $db_params['password'], $db_params['database']);
   if ($enlace->connect_errno) {
      $err_msg = '('.$enlace->connect_errno.'): '.$enlace->connect_error;
      _debug("Conexion BBDD [NOOK] phptype=>{$db_params['phptype']} username=>{$db_params['username']} password=>{$db_params['password']} hostspec=>{$db_params['hostspec']} database=>{$db_params['database']} || USERINFO = ".$err_msg,__LINE__,'ERR','connectDB');
      exit;
   }
   else {
      ...
      if ($enlace->errno != 0) {
			$err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
         echo "**ERROR**SET CHARACTER SET -> utf8 $err_str\n";
      }
   }
...
global $enlace;
   ...
   if ($enlace->errno != 0) {
      $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
      _debug("No se ha podido obtener la informacion de las tablas de la BBDD || USERINFO = ".$err_str,__LINE__,'ERR','DataInit');
		exit;
   }else{
   ...
   $tablasBBDD=array();
	while ($rTablas = $resultTablas->fetch_assoc()) {
...
global $enlace;
	...
	foreach ($DBProcedure as $id => $queryCreate){
	   ...
	   if ($enlace->errno != 0) {
   	   $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
			_debug("No se ha podido borrar el procedimiento $id||CMD=>$queryDrop || USERINFO = ".$err_str,__LINE__,'ERR','ProcedureInit');
		}
		else{
	      ...
	      if ($enlace->errno != 0) {
   	      $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
				_debug("No se ha podido crear el procedimiento $id||CMD=>$queryCreate || USERINFO = ".$err_str,__LINE__,'ERR','ProcedureInit');
			}
			else{
	...
	// Invocamos el procedure sp_alerts_read
   ...
   if (file_exists($FILE_SERVICES)) {
      ...
      $jdata = json_decode($content,true);
		$db_host = $jdata['cnm-data-db']['cnm']['db_host'];
		$local_ip = $jdata['cnm-data-db']['global']['cid_ip'];
	}
	else {	
   ...
   if ($enlace->errno != 0) {
      $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
        _debug("No se ha podido ejecutar el procedimiento $procedure||CMD=>$queryProcedure || USERINFO = ".$err_str,__LINE__,'ERR','ProcedureInit');
     }else{
...
global $enlace;
   ...
   $fp = popen($cmd, "r");
	...
	foreach ($all_scripts as $file) {
      ...
      $fdata = $enlace->real_escape_string(file_get_contents($fileFullPath));
      $queryInsert="INSERT INTO cfg_monitor_agent_script (script,size,date,script_data,custom) VALUES ('$file',$fsize,$fdate,'$fdata',0) ON DUPLICATE KEY UPDATE size=$fsize,date=$fdate,script_data='$fdata'";
	   ...
	   if ($enlace->errno != 0) {
   	   $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
         _debug("No se ha podido insertar los datos del script $file ||CMD=>$queryInsert || USERINFO = ".$err_str,__LINE__,'ERR','cfg_monitor_agent_script_update');
      }else{
	...
	while ($r_1 = $result_1->fetch_assoc()) {
      ...
      if ($enlace->errno != 0) {
         $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
         _debug("No se ha podido actualizar el campo signature del script $script||CMD=>$query_2 || USERINFO = ".$err_str,__LINE__,'ERR','cfg_monitor_agent_script_update');
      }else{
...
/*
   ...
   $result_1 = $enlace->query($query_1);
	while ($r_1 = $result_1->fetch_assoc()) {
		...
		$result_2 = $enlace->query($query_2);
		while ($r_2 = $result_2->fetch_assoc()) {
   ...
   $result_1 = $enlace->query($query_1);
	if($mode=0){
   	...
   	$result_2 = $enlace->query($query_2);
		...
		while ($r = $result_aux->fetch_assoc()) {
			$cont = $r['cuantos'];
   	}
...
*/
   ...
   $result_1_a = $enlace->query($query_1_a);
	while ($r_1_a = $result_1_a->fetch_assoc()) {
   ...
   foreach($in_bbdd_a as $script => $kk){
      if (!array_key_exists($script, $in_file_a)) {
         ...
         $result_2_a = $enlace->query($query_2_a);
			...
			while ($r_3_a = $result_3_a->fetch_assoc()) {
				$hparam = $r_3_a['hparam'];
   ...
   $result_1_b = $enlace->query($query_1_b);
	while ($r_1_b = $result_1_b->fetch_assoc()) {
...
*/
   ...
   $resultShowCreate=$enlace->query($sqlShowCreate);
 	if ($enlace->errno != 0) {
      $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
      _debug("No se ha podido obtener informacion de la tabla $nombreTabla || USERINFO = ".$err_str,__LINE__,'ERR','_checkTable');
      return;
   }else{
      _debug("Se ha obtenido la informacion de la tabla $nombreTabla",__LINE__,'DBG','_checkTable');
   }
	$rShowCreate = $resultShowCreate->fetch_assoc();
...
/*
      ...
      $resultCuantos=$enlace->query($sqlCuantos);
		$rCuantos = $resultCuantos->fetch_assoc();
*/
/*
      print ">>>>>>>>>$sqlCuantos<<<<<<<<<<\n";
		print_r($rCuantos);
...
*/
      ...
      foreach($tupla as $columna => $valor){
         if(is_array($valor)){
            ...
            $a_Cols[]=$columna;
            $a_Vals[]="'".$enlace->real_escape_string($valor)."'";

            $sqlBusqueda.=$sqlBusquedaSep.$columna.'='."'".$enlace->real_escape_string($valor)."'";
            $sqlBusquedaSep = "AND ";
         }
      }
      ...
      foreach ($tupla as $columna => $valor){
         ...
         $sqlUpdate.=$sqlUpdate_sep.$columna."='".$enlace->real_escape_string($valor)."'";
         $sqlUpdate_sep=',';
      }
...
/*	
      ...
      // SI EXISTE EL ELEMENTO => SE MODIFICA
         ...
         foreach ($tupla as $columna => $valor){
				if(in_array($columna,$a_pk)) continue;
				if(is_array($valor)) continue; // Tabla cfg_remote_alerts
				$sqlInsert.=$sqlInsert_sep.$columna."='".$enlace->real_escape_string($valor)."'";
				$sqlInsert_sep=',';
         }
...
*/
      ...
      $resultModificarTupla=$enlace->query($sqlTot);
	   if ($enlace->errno != 0) {
   	   $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
         _debug("Error al ejecutar $sqlTot (USERINFO = ".$err_str.")",__LINE__,'ERR','_DataInitTable');
      }
      ...
      if (array_key_exists('key', $struct)) {
         ...
         $resultGetId=$enlace->query($sqlGetId);
			$rGetId = $resultGetId->fetch_assoc();
         $last_id = $rGetId['id'];
         ...
         foreach ($struct['data'] as $b){
            ...
            $resultSqlCascade=$enlace->query($sqlCascade);
		      if ($enlace->errno != 0) {
      		   $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
		         _debug("Error al ejecutar $sqlCascade (USERINFO = ".$err_str.")",__LINE__,'ERR','_DataInitTable');
		      }
         }
      }
	}
...
global $enlace;
   ...
   if ($enlace->errno != 0) {
      $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
		_debug("No se ha podido obtener informacion de las tablas en la BBDD || USERINFO = ".$err_str,__LINE__,'ERR','SchemeInit');
		exit;
   }else{
...
/*
   ...
   foreach($tablasAEliminar as $TblAEliminar){
	   ...
	   if ($enlace->errno != 0) {
   	   $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
			_debug("No se ha podido borrar la tabla $TblAEliminar||CMD=>$sqlDelTabla||$err_str",__LINE__,'ERR','SchemeInit');
	   }else{
...
#PRIMARY KEY  (`id_dev`,`mtype`,`subtype`,`pattern`)
   ...
   if ($enlace->errno != 0) {
      $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
		_debug("No se ha podido eliminar la columna $ColAEliminar de la tabla $nombreTabla||CMD=>$sqlDelCol || USERINFO = ".$err_str,__LINE__,'ERR','_dropColumn');
   }else{
...
global $enlace;
   ...
   if ($enlace->errno != 0) {
      $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
		_debug("No se ha podido modificar la columna $nombreColumnaEst de la tabla $nombreTabla||CMD=>$sqlModify || USERINFO = ".$err_str,__LINE__,'ERR','_alterColumn');
   }else{
...
global $enlace;
   ...
   if ($enlace->errno != 0) {
      $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
		_debug("No se ha podido crear la columna $nombreColumnaEst de la tabla $nombreTabla||CMD=>$sqlAddCol || USERINFO = ".$err_str,__LINE__,'ERR','_addColumn');
   }else{
	...
	if(gettype($input_table)=='string'){
		...
		if(strpos($nombreTabla,'extra_')!==0){
	      ...
	      $resultDrop=$enlace->query($sqlDrop);
		   if ($enlace->errno != 0) {
      		$err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
	         _debug("No se ha podido eliminar la tabla $nombreTabla||CMD=>$sqlDrop || USERINFO = ".$err_str,__LINE__,'ERR','_dropTable');
	      }
			else{
...
global $enlace;
   ...
   if ($enlace->errno != 0) {
       $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
		_debug("No se ha podido crear la tabla $nombreTabla||CMD=>$sqlAdd || USERINFO = ".$err_str,__LINE__,'ERR','_addTable');
   }else{
...
*/
	...
	foreach ($datos as $nombreTabla => $infoTabla){
		...
		foreach($infoTabla['data'] as $tupla){
            ...
            $sqlTot.=$sep.$columna."='".$enlace->real_escape_string($valor)."'";
            $sep=',';
         }
			$sqlTot.=" WHERE ";
         ...
         $resultModificarTupla=$enlace->query($sqlTot);
		   if ($enlace->errno != 0) {
      		 $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
	         _debug("Error al ejecutar $sqlTot (USERINFO = ".$err_str.")",__LINE__,'ERR','DataModInit');
	      }
		}
	}
   ...
   foreach ($datos as $nombreTabla => $infoTabla){
      ...
      foreach($infoTabla['data'] as $tupla){
         ...
         $sep = '';
         foreach ($tupla as $columna => $valor){
            $sqlTot.=$sep."'".$enlace->real_escape_string($valor)."'";
				$sep=',';
         }
			$sqlTot.=")";
         // print "$sqlTot\n";
...
global $enlace;
	...
	foreach ($array_limpieza as $sql){
      ...
      if ($enlace->errno != 0) {
          $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
	      _debug("No se ha podido limpiar la tabla tips al ejecutar $sql || USERINFO = ".$err_str,__LINE__,'ERR','_limpiar_tips');
	      exit;
	   }else{
...
global $enlace;
   ...
   if ($enlace->errno != 0) {
      $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
		debug("No se ha podido obtener la informacion de las tablas de la BBDD || USERINFO = ".$err_str,__LINE__,'ERR','table_charset_latin1');
      exit;
   }else{
   ...
   foreach($tablasEst as $tabla){
      ...
      if ($enlace->errno != 0) {
         // $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
			// _debug("No se ha podido obtener informacion de la tabla $tabla".$err_str,__LINE__,'ERR','table_charset_latin1');
         continue;
      }else{
         // _debug("Se ha obtenido la informacion de la tabla $tabla",__LINE__,'DBG','table_charset_latin1');
      }

		$rCheckCharset = $resultCheckCharset->fetch_assoc();
	   // AHORA TENEMOS QUE DIVIDIR LAS LINEAS DENTRO DE UN ARRAY
	   $a_aux = array();
      ...
      if ($enlace->errno != 0) {
         $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
			...
			// FML REVISAR $resultAlterTabla->getCode()=='-18' --> ANTES NO SE HACIA NADA SI code=-18 ¿????

			_debug("No se ha podido cambiar el charset a latin1 de la tabla $tabla de la BBDD {$db_params['database']} || USERINFO = ".$err_str,__LINE__,'ERR','table_charset_latin1');
      }else{
...
global $enlace;
   ...
   if (file_exists($FILE_SERVICES)) {
      ...
      $a_client[]=array('hidx'=>1,'db1_user'=>$jdata['cnm-data-db']['onm']['db_user'],'db1_pwd'=>$jdata['cnm-data-db']['onm']['db_pwd'],'db1_server'=>$jdata['cnm-data-db']['onm']['db_host'],'db1_name'=>$jdata['cnm-data-db']['onm']['db_database'],'cid'=>'default','descr'=>'Default client');
   ...
   $result=$enlace->query("SELECT hidx,cid,descr,db1_name,host_ip,host_name,host_descr,id_client,status,mode FROM cfg_cnms WHERE host_ip='$local_ip'");

	if (mysqli_error($enlace) != 0) {
...
global $enlace;
   ...
   if ($enlace->errno != 0) {
      $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
      _debug("No se ha podido crear la BBDD cnm || USERINFO = ".$err_str,__LINE__,'ERR','_create_cnm_database');
      exit;
   }else{
...
/*   $result=$enlace->query('CREATE DATABASE IF NOT EXISTS onmgraph');
   if ($enlace->errno != 0) {
      $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
      _debug("No se ha podido crear la BBDD onmgraph || USERINFO = ".$err_str,__LINE__,'ERR','_create_onmgraph_database');
      exit;
   }else{
   ...
   connectDB($db_params);
	foreach ($a_client as $client){
      ...
      $db1_host = $client['db1_server'];
	   ...
	   if ($enlace->errno != 0) {
   	   $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
	      _debug("No se ha podido crear la BBDD $db1_name || USERINFO = ".$err_str,__LINE__,'ERR','_create_clients_databases');
	      exit;
	   }else{
...
/*
      ...
      if ($enlace->errno != 0) {
         $err_str = '('.$enlace->errno.'): '.$enlace->sqlstate.' - '.$enlace->error;
	      _debug("No se ha podido dar los privilegios al usuario $db1_user en la BBDD $db1_name ($qPrivileges) || USERINFO = ".$err_str,__LINE__,'ERR','_create_clients_databases');
	      exit;
	   }else{
...
//------------------------------------------------------------------------------------
// DB_SERVER = docker_cnm-data-db_1 -> hostspec
// DB_NAME = onm							-> database
// DB_USER = onm							-> username
// DB_PWD = onm1234						-> password
// DB_TYPE = mysql						-> phptype

function get_db_credentials_all(){
   ...
   if (file_exists($FILE_SERVICES)) {
      ...
      $jdata = json_decode($content,true);
   	...
   	$dbparams['database'] = 'cnm';
		$dbparams['hostspec'] = $jdata['cnm-data-db']['cnm']['db_host'];
		$dbparams['username'] = $jdata['cnm-data-db']['cnm']['db_user'];
		$dbparams['password'] = $jdata['cnm-data-db']['cnm']['db_pwd'];
		$dbparams['phptype'] = $jdata['cnm-data-db']['cnm']['db_type'];

	   if (! array_key_exists('database', $dbparams)) { $dbparams['database']='cnm'; }
   	if (! array_key_exists('hostspec', $dbparams)) { $dbparams['hostspec']='localhost'; }
   	if (! array_key_exists('username', $dbparams)) { $dbparams['username']='onm'; }
   	if (! array_key_exists('password', $dbparams)) { $dbparams['password']='onm1234'; }
   	if (! array_key_exists('phptype', $dbparams)) { $dbparams['phptype']='mysql'; }
   ...
   while (!feof($fp)) {
      ...
      if(strpos($line,'=')!==false){
         ...
         $name  = trim($name);
         $value = trim($value);
         if ($name=='DB_SERVER') { $dbparams['hostspec']=$value; }
         elseif ($name=='DB_NAME') { $dbparams['database']=$value; }
         elseif ($name=='DB_USER') { $dbparams['username']=$value; }
         elseif ($name=='DB_PWD') { $dbparams['password']=$value; }
         elseif ($name=='DB_TYPE') { $dbparams['phptype']=$value; }
      }
   }
   fclose($fp);

	// FML -> La BBDD tiene que ser cnm !!!
	$dbparams['database']='cnm';

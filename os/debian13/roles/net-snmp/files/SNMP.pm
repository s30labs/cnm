...
sub _gettable_do_it() {
	...
	my $mib_base='';
 	if ($textnode =~ /^(\S+)\:\:\S+$/) { $mib_base=$1; }

 	my $end_of_view=0;
	for (my $i = 0; $i <= $#$vbl; $i++) {
		 my $oidx = ($mib_base ne '') ? $mib_base.'::'.$vbl->[$i][0] : $vbl->[$i][0];
	    #my $row_oid = SNMP::translateObj($vbl->[$i][0]);
		 my $row_oid = SNMP::translateObj($oidx);
	    my $row_text = $vbl->[$i][0];
	    ...
	    my $row_type = $vbl->[$i][3];
		 ...
		 #FML Si no se pone puede quedarse en bucle en casos particulares
	    if ($row_value =~ /No Such Object available on this agent at this OID/) {
 				next;
 		 }
 		 if ($row_value =~ /No more variables left in this MIB View/) {
	        $end_of_view=1;
 		 }


	    if (defined $row_oid && $row_oid =~ 

#####################################################################################################
# Fichero: ProvisionLite::pkts_type_mibii_if.pm
# Set Tab=3
#####################################################################################################
package ProvisionLite::pkts_type_mibii_if;
$VERSION='1.00';
use strict;
use Crawler::SNMP;
...
my %snmpcfg=();
   ...
   $snmpcfg{'oid'}='ifDescr_ifInUcastPkts_ifInNUcastPkts_ifOutUcastPkts_ifOutNUcastPkts';
...
#      $data_global->{'label'}="Tipo de trafico U/NU en $descr (".$device->{'full_name'}.')' ;
#      $data_global->{'items'}="Pkts unicast IN ($id)|Pkts no unicast IN ($id)|Pkts unicast OUT ($id)|Pkts no unicast OUT ($id)";
